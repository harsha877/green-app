...
import {
	CUSTOM_QUESTION_HEAD_TAG,
	CUSTOM_QUESTION_NEXT_TAG,
	CUSTOM_QUESTION_SUBMIT_TAG,
...
export default function QuizInput({ navigation, route }) {
	...
	function handleNext() {
		...
		if (
			checkLenghtEqualToZero(question) ||
			checkLenghtEqualToZero(option1) ||
			checkLenghtEqualToZero(option2) ||
			checkLenghtEqualToZero(option3) ||
			...
			{flag == false ? (
				<>
					...
					<CustumeButton
						...
						onPressHandler={onSubmitQuizName}
						name="Submit"
					/>
				</>
			) : (
				<>
					...
					<View style={{ flexDirection: "row" }}>
						...
						<Picker
							onValueChange={(itemValue, itemIndex) =>
								priorityChangeHandler(itemValue, "q1")
							}
							style={styles.picker}
							selectedValue={newpriority[0]}
						>
							{priority.map((key) => (
								<Picker.Item
									label={key}
					...
					<View style={{ flexDirection: "row" }}>
						...
						<Picker
							onValueChange={(itemValue, itemIndex) =>
								priorityChangeHandler(itemValue, "q2")
							}
							style={styles.picker}
							selectedValue={newpriority[1]}
						>
							{priority.map((key) => (
								<Picker.Item
									label={key}
					...
					<View style={{ flexDirection: "row" }}>
						...
						<Picker
							onValueChange={(itemValue, itemIndex) =>
								priorityChangeHandler(itemValue, "q3")
							}
							style={styles.picker}
							selectedValue={newpriority[2]}
						>
							{priority.map((key) => (
								<Picker.Item
									label={key}
					...
					<View style={{ flexDirection: "row" }}>
						...
						<Picker
							onValueChange={(itemValue, itemIndex) =>
								priorityChangeHandler(itemValue, "q4")
							}
							style={styles.picker}
							selectedValue={newpriority[3]}
						>
							{priority.map((key) => (
								<Picker.Item
									label={key}
