...
export default function QuizInput({setUser})
{
   ...
   function handleSubmit(e)
   {
       ...
       data.unshift(obj);
       console.log(data);
       for(let i=0;i<data.length;i++) writeCollection(data[i],`${i}`,id).then((response)=>console.log(response)).catch((e)=>console.log(e))
       setData([]);
       setUser("");
   }
  return(
    <View style={styles.container}>
       {flag==false?<>
       <TextInput style={styles.TextInput} placeholder="Enter ID" value={id} onChangeText={(text) => setId(text)} />
